...
function Home() {
  ...
  useEffect(() => {
    ...
    tick();
    return () => { clearInterval(id); document.title = 'JishinWatch'; };
  }, []);
  // ────────────────────────────────────────────────────────────────────────
    ...
    <div className="relative w-full h-screen bg-black overflow-hidden font-sans text-white dark">
      ...
      {showTerms && (
        <div className="fixed inset-0 z-[500] flex items-center justify-center bg-black/80 backdrop-blur-sm p-4">
          <div className="glass-panel rounded-2xl max-w-3xl w-full max-h-[85vh] overflow-y-auto custom-scrollbar p-5 flex flex-col gap-4">
            <div>
            ...
            <div className="grid sm:grid-cols-2 gap-4">
              <div className="rounded-lg border border-yellow-500/30 bg-yellow-500/10 p-3 text-[12px] leading-relaxed text-yellow-100/90">
                本アプリは気象庁・国土交通省・国土地理院その他の公的機関が提供する公式サービスではありません。個人がAI（Claude）を活用して開発した非公式の地震情報表示アプリです。緊急地震速報・地震情報・津波情報は、必ず気象庁など公式の発表・報道機関の情報もあわせてご確認ください。本アプリの情報の正確性・即時性は保証されません。人命・避難判断・医療・交通等に関わる緊急の判断において、本アプリを唯一の情報源として使用しないでください。
              </div>
            ...
            <a
              href="https://github.com/26good/JishinWatch/blob/main/TERMS.md"
              target="_blank"
        ...
        {leftTab === 'settings' && (
          <div className="glass-panel flex-grow rounded-xl pointer-events-auto overflow-y-auto custom-scrollbar p-4 flex flex-col gap-5">
            ...
            <div className="border-t border-white/8 pt-4 mt-auto flex flex-col items-center gap-2">
              ...
              <div className="text-[11px] text-white/25 text-center">JishinWatch Ver 2.4.0</div>
            </div>
          </div>
        )}
